...
function ignoreThisPage(options) {
    return $.grep(options.except_urls, function(url) {
        return RegExp(url).test(window.location.href);
    }).length > 0;
}

function whiteList(options) {
    return $.grep(options.only_urls, function(url) {
            return RegExp(url).test(window.location.href);
        }).length > 0 ||
...
chrome.extension.sendRequest({
    ...
    function process(e) {
        ...
        if (/INPUT|TEXTAREA/.test(hit_elem.nodeName) || hit_elem.isContentEditable || $(hit_elem).parents().filter(function() {
